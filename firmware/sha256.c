...
void sha256_block_dump(SHA256_CTX *ctx, unsigned char data[]){
	char block_string[BLOCK_STRING_SIZE+1];
	char string[3];

	for (int i = 0; i < BLOCK_CHAR_SIZE; i++) {
		char_to_hex_string(data[i], string);

        block_string[i*2] = string[0];
		block_string[i*2+1] = string[1];
	}
...
void sha256_update(SHA256_CTX *ctx, unsigned char data[], unsigned int len)
{
	for (int i = 0; i < len; ++i) {
		...
		if (ctx->datalen == BLOCK_CHAR_SIZE) {
			...
			sha256_block_dump(ctx, ctx->data);
